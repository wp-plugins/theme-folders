...
<h2><img src="<?php
    echo $icons_url = get_bloginfo('url') . "/wp-content/plugins/theme-folders/icons/";
    echo "folder32.png";
?>" /><?php
    _e('Theme Folders');
?></h2>
...
Theme Folders <input type="text" value="<?php
    echo $folder_vars;
?>" size=50 name="design_folders" /><small>(divided by comma + space ", ")</small><br />
...
<?php
    ...
    foreach ($wp_theme_directories as $themes) {
        ...
        foreach ($my_theme_folders as $theme) {
            ...
            echo "<img style='height:20px;' src='" . $icons_url . "document-save.png' /></a> ";
            echo "<a href='" . get_bloginfo('url') . "/wp-admin/customize.php?theme=" . $themefolder2[$last - 1] . "' >";
            echo "<img style='height:20px;' src='" . $icons_url . "edit-find.png' /></a>";
            
            echo "<form style='height:20px;display:inline;' action='../wp-admin/theme-editor.php' method='post' />";
            echo "<input type=\"hidden\" name=\"theme\" value=\"" . $themefolder2[$last - 1] . "\" />";
            echo "<input type='image' name='cmdSubmit' alt='Submit Form' src='" . $icons_url . "document-properties.png' value='Submit' style='height:26px;vertical-align:bottom;' /></form>";
            ...
            echo "<img style='height:20px;' src='" . $icons_url . "edit-delete.png' /></a>";
